...
class MALMallCartListView: UIView {
    ...
    var isUnSelectAll:Bool = false {
        
        didSet {
            
            if(isUnSelectAll == false){
                
                let count = dataList?.count ?? 0
                
                for index in 0..<count {
                    
                    var item:MALCarRecordsModel = dataList?[index] ?? MALCarRecordsModel.init()
    ...
    /// 修改购物车商品数量
    var changeItemContBlock: (() -> Void)?

    /// Block: 删除无效商品
    var deleteInvalGoodsBlock: (() -> Void)?
    
    var currentCarModel: MALCartListModel?

    
    var dataList: [MALCarRecordsModel]? {
    ...
    private var tempSelectItems:[IndexPath] = [IndexPath]() {
        didSet {
            let isAll = tempSelectItems.count == dataList?.count
    ...
    func loadCartInfo(_ cartModel: MALCartListModel?) {
        
        dataList =  cartModel?.records
        currentCarModel  = cartModel
    ...
    lazy var carViewModel: MALShoppingCarViewMolde = {
        
        let viewModel = MALShoppingCarViewMolde.init()
        return viewModel
...
extension MALMallCartListView{
    ...
    func createRefresh() {
        ...
        listView.mj_header = MJRefreshNormalHeader.init(refreshingTarget: self, refreshingAction: #selector(p_loadData))
        listView.mj_footer = MJRefreshBackNormalFooter.init(refreshingTarget: self, refreshingAction: #selector(p_loadMoreData))
    }
    ...
    func reqeustListByPage(_ page: Int) {
        ...
        carViewModel.getShoppingCartList(currentPage: tempPage, pageSize: 10) { [self] (item) in
       
            if item?.records.isEmpty == true {
                isNomoreData = true
                listView.endRefresh()
                return
            }
            ...
            if page == 1 {
                isNomoreData = false
                dataList?.removeAll()
                dataList = item?.records
            } else {
                dataList! += item?.records ?? [MALCarRecordsModel.init()]
            }
        }
    }
...
extension MALMallCartListView {
    ...
    func getListHeight() -> CGFloat {
        ...
        let validCount = dataList?.count ?? 0
    ...
    func getSelectGoodsIDs() -> String {
        var goodids = ""
        if let cartList = dataList, tempSelectItems.isEmpty == false {
    ...
    /// 获取当前选择的商品列表
    func getSelectItems() -> [MALCarRecordsModel] {

        var tempArr = [MALCarRecordsModel]()
        let cartList = dataList
        _ = tempSelectItems.filter { (index) -> Bool in
    ...
    func getSelectedGoodsPrice() -> Double {
        ...
        _ = tempSelectItems.filter { (index) -> Bool in
            let item = cartList?[index.row]
            if let price =  Double(item?.price ?? "0"), let count = item?.num?.int {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            cell.changeItemSelectedBlock = { [self] (select) in
                if tempSelectItems.contains(indexPath) {
                    ...
                    self.dataList?[indexPath.row] = model ?? MALCarRecordsModel.init()
        ...
        cell.removeItemBlock = { [weak self] in
            
            self?.dataList?.remove(at: indexPath.row)
            
        }
        cell.changeItemCountBlock = {[weak self] (num,goodID) in
            
            var model = self?.dataList?[indexPath.row]
                model?.num  = num
            self?.dataList?[indexPath.row] = model!
            
            guard let block = self?.changeItemContBlock  else {
        ...
        if let model = dataList?[indexPath.row] {
               ...
               var currentModel = dataList?[indexPath.row]
            
            if(isSelectAll == true){
              
                currentModel?.iSSelected = isSelectAll
                if tempSelectItems.contains(indexPath) {
              ...
              cell.loadInfo(currentModel ?? MALCarRecordsModel.init())
              cell.currentCarModel = currentCarModel
...
class SZPMallCartCell: UITableViewCell {
    ...
    var changeItemCountBlock: ( (_ num: String?, _ goodsID: String?) -> Void )?
    var removeItemBlock:(() -> Void)?
    ...
    func loadInfo(_ model: MALCarRecordsModel) {

        tempGoodsID = model.id?.string

        if let img = model.imgUrl {
        ...
        selectIconBtn.isSelected = model.iSSelected
    ...
    @objc private func p_clickReduceIcon(_ sender:UIButton) {
        ...
        //减
        var count = countLabel.text?.int ?? 0
        
        count -= 1
        
        //加
        carViewModel.changeShopCarGoodsNum(goodsId: tempGoodsID?.int ?? 0, num: count ) {[weak self] code, messge in
        
            
            if(code == 100){
               
                self?.countLabel.text = String(format: "%d", count)
                guard let block = self?.changeItemCountBlock else { return }
                block(self?.countLabel.text, self?.tempGoodsID)
                
            }else{
                
                SZPHudExtension.szp_showPlainText(message:messge)
    ...
    @objc private func p_clickDel(_ sender:UIButton){
        ...
        carViewModel.removeShopCarGoodsNum(goodsId: tempGoodsID?.int ?? 0) {[weak self] code, messge in
        
            
            if(code == 100){
               
                guard let block = self?.removeItemBlock else {
                    return
                }
                block()
