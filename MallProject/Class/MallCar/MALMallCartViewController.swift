...
class MALMallCartViewController: MALBaseViewController {
    ...
    func  requestCartList()  {
        ...
        carViewModel.getShoppingCartList(currentPage: 1, pageSize: 10) { [weak self] model in
            
          
            self?.cartListView.loadCartInfo(model)
    ...
    override func viewDidLoad() {
        ...
        cartListView.changeItemContBlock = { [self] in
            
   
            let price = cartListView.getSelectedGoodsPrice()
            bottomPayView.loadSelectedPrice(price)
            
        }
        
        cartListView.selectAllStateBlock = { [self] (isAll,count) in
            
            if(count > 0){
                
             bottomPayView.isCanBtn(true)
            
            }else{
            
              bottomPayView.isCanBtn(false)
                
            }
          
            bottomPayView.refreshAllStateBTN(isAll)
            let price = cartListView.getSelectedGoodsPrice()
            bottomPayView.loadSelectedPrice(price)
        }
        
        bottomPayView.cartBottomActionBlock = {  [self] (type) in

            switch type {
            case .typeOfAllSelect:
                
                cartListView.isSelectAll = true
                bottomPayView.isCanBtn(true)
                let price = cartListView.getSelectedGoodsPrice()
                bottomPayView.loadSelectedPrice(price)
            ...
            case .typeOfNoSelect:
                ...
                cartListView.isUnSelectAll = false
                bottomPayView.isCanBtn(false)
...
class SZPCartBottomView: UIView {
    ...
    private lazy var priceLabel: UILabel = {
     ...
     lazy var payBtn: UIButton = {
        ...
        view.backgroundColor = UIColor.init(hexColor: "#CCCCCC")
        view.setBackgroundImage(UIImage.init(color: UIColor.AppColor.main, size: CGSize(width: MAL_Value(118), height: MAL_Value(48))), for: .selected)
